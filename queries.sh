...
echo "$($PSQL "SELECT name FROM teams JOIN games ON teams.team_id = games.winner_id WHERE round = 'Eighth-Final' and year = 2014 UNION ALL SELECT name FROM teams JOIN games ON teams.team_id = games.opponent_id WHERE round = 'Eighth-Final' and year = 2014 ORDER BY name")"
